...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create event-manager-31a5
  fly apps create event-manager-31a5-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app event-manager-31a5
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app event-manager-31a5-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'event-manager-31a5-staging' does not match app name in config file 'event-manager-31a5'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name event-manager-31a5-db
  fly postgres attach --app event-manager-31a5 event-manager-31a5-db

  fly postgres create --name event-manager-31a5-staging-db
  fly postgres attach --app event-manager-31a5-staging event-manager-31a5-staging-db
  ```
